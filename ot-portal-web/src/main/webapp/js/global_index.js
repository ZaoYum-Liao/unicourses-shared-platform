...
function isOnline(home_url,passport_url){
	...
	$.getJSON( "/ajax/isOnline/?callback=?", function( data ) {
		if (data.welcome){
			...
			$('#login').html(welComeMsg+''+passport_url+'<a href="/?returnUrl=">请登录</a>&nbsp;&nbsp;<a href="'+escape(document.location.href)+''+passport_url+'/reg/?returnUrl='+escape(document.location.href)+'">免费注册</a>');
		}
		if(data.qqcb){
